...
def popularAuthor(cur):
    cur.execute('''select name, sum(res3.count) as num
...
def errorPercent(cur):
    cur.execute('''select res.day, (res.failed::decimal/res.total)*100
        as error from (
...
if __name__ == '__main__':
    ...
    popularAuthor(cur)
    print("> Failed requests more than 1 percent on followiing days:\n")
    errorPercent(cur)
    db.close()
